...
__attribute__((visibility("default"))) static char *MAPPED_DEVICE_NAME = "dmx";

char mjsb_magic[16] = {0x6c, 0x69, 0x6c, 0x79, 0x6d, 0x75, 0x66, 0x66, 0x69, 0x6e, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00};
 ...
 * @param out[out] Null terminatwwed hex string of bytes
 */
 ...
 * @param blocks_per_level(NEW) Array containing the blocks per level at index of level, is returned by compute_block_numbers
 *
 * @return Non zero value means error, else 0
 */
int compute_hash_blocks(uint64_t data_blocks, uint32_t fanout, uint32_t *levels, uint32_t *hash_blocks, uint32_t *blocks_per_level)
{
	*levels = 0;
	*hash_blocks = 0;
						  ...
						  uint32_t *blocks_per_level, uint32_t hash_bytes)
{
	...
	*pad_blocks = blocks;
	...
	// what do low mid and high do?

	uint64_t low = 0;
	uint64_t high = blocks;
	// bpl = block per level, working variable
	...
	while (high >= low && high != 0)
	{
		...
		// Result is better
		// What does 'Result is better' mean?
		if (used <= blocks && pb < *pad_blocks)
		{

			// can be left out, is returned
			*data_blocks = db;
			*hash_blocks = hb;
			// jb = journal_block = *jb_blocks?
			*jb_blocks = jb;
			*pad_blocks = pb;
			*levels = lev;

			// just fill all the block from compute_hash_blocks into return array
...
int main(int argc, char const *argv[])
{
	...
	{
		warn("File is not a multiple of block_size: %d. %ju bytes left over", block_size, file_stats.st_size % block_size);
	}
	if (two_disks && S_ISREG(file_stats2.st_mode) && file_stats2.st_size % block_size != 0)
	{
		warn("File is not a multiple of block_size: %d. %ju bytes left over", block_size, file_stats.st_size % block_size);
	}
	...
	// What does 'TODO: uh...this is 64 bits...' mean?
	...
	if (!two_disks)
	{
		printf("[DEBUG] compute block numbers:\n");
		printf("Input: blocks: %ld\n\t, block_size: %d\n\t, fanout: %d\n\t, journal_blocks: %d\n\t, hash_blocks: %d\n\t, jb_blocks: %d\n\t, pad_blocks: %d\n\t, levels: %d\n\t, blocks_per_level %ls\n\t, hash_bytes %d\n",
			   blocks, block_size, fanout, journal_blocks, hash_blocks, jb_blocks, pad_blocks, levels, blocks_per_level, hash_bytes);
		...
		printf("\n[DEBUG] compute block numbers:\n");
		printf("Output: blocks: %ld\n\t, block_size: %d\n\t, fanout: %d\n\t, journal_blocks: %d\n\t, hash_blocks: %d\n\t, jb_blocks: %d\n\t, pad_blocks: %d\n\t, levels: %d\n\t, blocks_per_level %ls\n\t, hash_bytes %d\n",
			   blocks, block_size, fanout, journal_blocks, hash_blocks, jb_blocks, pad_blocks, levels, blocks_per_level, hash_bytes);

		printf("[DEBUG] block per level\n");
		for (int i = 0; i < levels; i++)
			printf("\tblocks_per_level[%d] = %d\n", i, blocks_per_level[i]);
	}
	else
	{
	...
	// Result info
	info("Blocks: %ju = Superblock: 1, Data: %ju, Hash: %u, JB: %u, Pad: %u, Levels: %u", blocks, data_blocks, hash_blocks, jb_blocks, pad_blocks, levels);
	printf("Blocks per level: ");
	for (int i = 0; i < levels; i++)
	{
		printf("block_per_level[%d] = %d\n", i, blocks_per_level[i]);
	}
	...
	// Data hash
	// hash zero block
	hash(md_hash, mdctx_hash, zero_block, block_size, salt, strlen(salt_str) / 2, hash_output, &hash_length);

#ifdef DEBUG
	printf("[DEBUG] hash zero block: [");
	for (int i = 0; i < hash_length; i++)
	{
		// printf("0x%x, ", (unsigned char)hash_output[i]);
		printf("%d, ", (unsigned char)hash_output[i]);
	}
	printf("\b\b]\n");
#endif

	// Now loop through each level, create the merkle tree by hashing the hashes
	for (uint32_t i = 0; i < levels; i++)
	{
		...
		{
			// b = byte in hash_bytes => number of bytes to hash
			for (int b = 0; b < hash_bytes; b++)
			{
				// isn't 1 << fls the fanout value?
				hash_levels[i][f * (block_size / (1 << fls)) + b] = hash_output[b];
				// own debug print statement
				// printf("saved = hash_output[%d] = %d to hash_levels[%d][%d + %d]\n", b, hash_output[b], i, f * (block_size / (1 << fls)), b);
			}
		}
		// Compute hash of this level for next iteration/root
		hash(md_hash, mdctx_hash, hash_levels[i], block_size, salt, strlen(salt_str) / 2, hash_output, &hash_length);
#ifdef DEBUG
		printf("[DEBUG] hash at level %d: [", i);
		for (int i = 0; i < hash_length; i++)
		{
			printf("0x%x, ", (unsigned char)hash_output[i]);
			// printf("%d, ", (unsigned char)hash_output[i]);
	...
	// Write it out!
	// The final superblock will take up one entire BLOCK, aka 512 / 4096 ... bytes
	...
	// magic value as delimiter
	// char *delim = "TEST TEST TEST";
	// write(file, delim, strlen(delim));
	...
	uint8_t p = 0;
	// why define p here? -> for the progress bar...
	...
	struct mint_journal_header *mjh = (struct mint_journal_header *)malloc(sizeof(struct mint_journal_header));
	...
	// p is just for progres, ignore and replace with rust progress bar if trivial, if not just ignore
	p = 0;
	...
	if (zero)
	{
		...
		for (uint64_t i = 0; i < data_blocks / multiple; i++)
		{
			...
			{
				exit_error_f("Failed to write data block: %ju, %s", i, strerror(errno));
			}
		...
		for (uint64_t i = 0; i < data_blocks % multiple; i++)
		{
			...
			p = progress(data_blocks - (data_blocks % multiple) + i + 1, data_blocks, 79, p);
		}
		fprintf(stderr, "\n");
	}
	else
	...
	printf("dmsetup create %s --table \"%u %ju dmx %s %s %u %u %u %ju "
		   "%s %s %s %s %s%s %s\"\n",
		   MAPPED_DEVICE_NAME,
		   0,
